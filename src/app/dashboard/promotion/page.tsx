...
import PromotionsPricingPanel from './PromotionsPricingPanel'
...
export default function PromotionAdminPage() {
  ...
  const setPrice = async (id: number) => {
    ...
    const raw = window.prompt(
      `Set price for “${promo?.title || `promo #${id}`}” (MWK)`,
      String(promo?.price ?? ''),
    )
    if (raw == null) return
    const price = Number(String(raw).replace(/[^\d.]/g, ''))
    if (!Number.isFinite(price) || price < 0) {
      setError('Enter a valid MWK price')
      return
    }
    setError('')
    setNotice('')
    try {
      const res = await adminFetch(`/api/admin/promos/${id}`, {
        method: 'PATCH',
      ...
      setPromos(prev => prev.map(p => (p.id === id ? { ...p, price: Math.round(price) } : p)))
      setNotice(`Price updated to ${formatMwk(Math.round(price))}`)
    } catch (err) {
  ...
  const activate = async (id: number) => {
    ...
    if (
      !(await confirm({
        title: 'Activate?',
        message: `Activate “${promo?.title || `promo #${id}`}”?\n\nIt will show again in the app.`,
    ...
    setNotice('')
    try {
      const res = await adminFetch(`/api/admin/promos/${id}/activate`, { method: 'PATCH' })
      const data = await res.json()
      if (!res.ok) throw new Error(data.error || 'Activate failed')
      setPromos(prev => prev.map(p => (p.id === id ? { ...p, isActive: true } : p)))
      setCounts(c => ({
        all: c.all,
        live: c.live + 1,
        inactive: Math.max(0, c.inactive - 1),
      }))
      setNotice('Promotion activated')
      setTab('live')
    } catch (err) {
  ...
  return (
    <div>
      ...
      <DashboardPageHeader
        sectionId="promotion"
        description="Set merchant promo price presets, review live offers, and override prices shown in the app."
        actions={
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            <button
              ...
              style={{
                ...
                background: view === 'pricing' ? 'var(--primary)' : '#fff',
                color: view === 'pricing' ? '#fff' : 'var(--text-2)',
                fontWeight: 700,
                fontSize: 13,
              }}
            >
            ...
            <button
              ...
              style={{
                ...
                background: view === 'list' ? 'var(--primary)' : '#fff',
                    ...
                    >
                      ...
                      <div>
                        ...
                        <div style={{ fontSize: 12, color: 'var(--text-4)' }}>
                          {formatMwk(promo.price)} · {formatPromoDate(promo.startDate)} →{' '}
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                        <button
                          type="button"
                          onClick={() => void setPrice(promo.id)}
                          style={{
                            padding: '8px 12px',
                            borderRadius: 10,
                        ...
                        {promo.isActive ? (
                          ...
                          <button
                            type="button"
                            onClick={() => void activate(promo.id)}
                            style={{
                              padding: '8px 12px',
                              borderRadius: 10,
                              border: 'none',
